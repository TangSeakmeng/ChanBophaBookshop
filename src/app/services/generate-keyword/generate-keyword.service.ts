...
export class GenerateKeywordService {
...
export const createKeywords = (name: string) => {
  ...
  name.trim().split(/[ .\-_\s ]/).forEach(letter => {
    ...
    return (letter.split('').forEach(word => {
      nextWord += word;
      arrName.push(GenerateKeywordService.toCapitalize(nextWord));
    }))
  })
  name.trim().split('').forEach(word => {
    nextLetter += word;
    arrName.push(GenerateKeywordService.toCapitalize(nextLetter));
  })
  return arrName
}
